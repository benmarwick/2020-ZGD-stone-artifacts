...
all_lithics %>% 
  mutate(bipolar = `bipolar core` + `bipolar flake`)  %>% 
  select(layer...3, 
         cores = `core total`,           
         flakes =  `flake total`,           
        `retouched\npieces` = `tool total`,
...
# reading ages off the strat plot
...
ages2 = BchronCalibrate(ages =   c14_ages_clean$age,
                        ageSds = c14_ages_clean$error,
                        calCurves=rep('intcal20', 
                                      nrow(c14_ages_clean)),
                        ids = c14_ages_clean$Lab.no.,
...
# use this one, with the strat plot, add climate plot using
# https://github.com/benmarwick/gsloid
